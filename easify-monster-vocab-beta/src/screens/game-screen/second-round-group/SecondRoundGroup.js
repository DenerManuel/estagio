...
import { createCardContainer, positionCardContainer } from "./components/card-container/cardContainerManager.js";
...
export class SecondRoundGroup extends PIXI.Container {
  ...
  _setupRound(index = null) {
    ...
    if (this.currentRound < 17) {
      ...
      this.currentCardNames = getNames(availableNames, this.currentRound, this.currentCardNames, index)
    ...
    if (this.currentRound % 2 === 0) {
      this.currentContainerNames = getUniqueRandomNames(this.currentCardNames, this.containersOnScreen);
    }
    // } else if (this.currentRound === 19 && this.currentCardNames.length > 0) {
    //   const containerName1 = this.currentCardNames[0];
    //   const availableForContainer2 = this.allNames.filter(n => n !== containerName1);
    //   const containerName2 = getUniqueRandomNames(availableForContainer2, 1)[0] || containerName1;
    //   this.currentContainerNames = [containerName1, containerName2];
    // }


    // // Caso especial para última rodada
    // if (this.currentRound === 19 && this.currentCardNames.length > 0) {
    //   const containerName1 = this.currentCardNames[0];
    //   const availableForContainer2 = this.allNames.filter(n => n !== containerName1);
    //   const containerName2 = getUniqueRandomNames(availableForContainer2, 1)[0] || containerName1;
    //   this.currentContainerNames = [containerName1, containerName2];
    // } else {
    //   // Containers são sempre sorteados dentre os cards disponíveis
    //   this.currentContainerNames = getUniqueRandomNames(this.currentCardNames, this.containersOnScreen);
    // }
  ...
  _handleCardDrop(card) {
    ...
    const targetContainer = this.containerRefs.find(ref =>
      this._isOverlapping(card, ref.cardContainer) && ref.cardContainer.name === card.name
    );
