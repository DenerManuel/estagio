...
export function configurePreGameScreenButtons(screenManager) {
  ...
  _configurePreGameScreenPlayButton(screenManager);
}
...
const _configurePreGameScreenPlayButton = (screenManager) => {
  const preGameScreen = screenManager.screens.preGameScreen;
  const playButton = preGameScreen.playButton;
  
  playButton.on('pointertap', () => {
    screenManager.displayScreen('gameScreen', preGameScreen.category);
  });
}