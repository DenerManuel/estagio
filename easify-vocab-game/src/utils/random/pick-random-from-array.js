...
export function pickRandomItemsExcluding(allItems, totalToSelect, excludedItems) {
  ...
  // Calcula quantos itens aleatórios precisam ser selecionados.
  const numberOfItemsToPick = Math.max(0, totalToSelect);

  // Usa a função existente para pegar os itens aleatórios da lista de disponíveis.
  return pickRandomFromArray(availableItems, numberOfItemsToPick);
}
